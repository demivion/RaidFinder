...
function EnKai.doc (name, parent)
	...
	local grid = EnKai.uiCreateFrame("nkGrid", name .. '.grid', doc:GetContent())
		
	grid:SetHeaderHeight(0)
	grid:SetTransparentHeader(true)
	grid:SetFontSize(13)
	grid:SetPoint("TOPLEFT", doc:GetContent(), "TOPLEFT")	
	...
	local slider = EnKai.uiCreateFrame("nkScrollbox", name .. ".slider", doc:GetContent())	
	...
	local scrollPane = EnKai.uiCreateFrame("nkScrollpane", name .. '.body', doc:GetContent())
	scrollPane:SetPoint("TOPLEFT", grid, "TOPRIGHT")
	scrollPane:SetWidth(264)
	scrollPane:SetHeight(grid:GetHeight())
	
	
	local body = UI.CreateFrame("Frame", name .. '.body', doc:GetContent())
	...
	function doc:AddChapter(parentChapter, newTitle, newContent, updateFlag)
		...
		if updateFlag == true then doc:UpdateGrid() end		
	end
	
	function doc:GetFreeElement(type)
	...
	function doc:CreateElement (type)
	...
	function doc:ShowChapter (key)
		...
		for k, entry in pairs (content[key]) do
			...
			local widget = doc:GetFreeElement(entry.type)
			if widget == nil then widget = doc:CreateElement(entry.type) end
			if widget ~= nil then
	...
	Command.Event.Attach(EnKai.events[name .. ".grid"].LeftClick, function (_, rowNo)
		doc:ShowChapter(grid:GetKey(rowNo))
